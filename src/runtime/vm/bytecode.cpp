        ...
        const StringData* cns)
{
    ...
    if (class_ == NULL)
    {
        if(cls->data()[0]=='\\')
        {
            StringData *tmp = new StringData(cls->data()[0]+1);
            class_ = Unit::loadClass(ne,tmp);
            delete tmp;
        }
        if(class_==NULL)
        {
                                ...
                                bool decRef = false)
{
    ...
    if (IS_STRING_TYPE(input->m_type))
    {
        ...
        if (class_ == NULL)
        {
            const char *tmp = input->m_data.pstr->data();
            if(tmp[0]=='\\')
            {
                tmp++;
            }
                        StringData *tmpString = new StringData(tmp);
            class_ = Unit::loadClass(tmpString);
            delete tmpString;
            if(class_==NULL)
            {
